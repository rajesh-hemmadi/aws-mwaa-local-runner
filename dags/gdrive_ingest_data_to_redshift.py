...
def get_gdrive_metadata_from_metadataSheet():
    try:
        ...
        file_metadata_columns = ['id', 'source_folder_path', 'source_file_type', 'source_file_delimiter', 'source_sheet_name', 'top_rows_to_skip',
                                 'pre_processing_script', 'is_active', 'source_folder_name', 'target_table', 'target_db', 'target_schema', 'unique_load_name']
...
def read_s3_csv_first_row(bucket_name, key):
    try:
        print(bucket_name)
        print(key)
        ...
        print('Object')
        print(obj)
        first_line = obj.get()['Body'].readline().decode("utf-8")
        print('First Line')
...
def return_load_process_reference_id(**kwargs):
    try:
        redshift_hook = RedshiftSQLHook(redshift_conn_id=REDSHIFT_CONN_ID)
        schema_name = kwargs.get('schema_name')
        sp_name = kwargs.get('sp_name')
...
def transfer_file_from_gdrive_to_s3(**kwargs):
    try:
        ...
        download_from_gdrive_to_local = GoogleDriveToLocalOperator(
            ...
            output_file='{}/{}'.format(local_path, file_name)
        )
        ...
        s3_upload_operator = LocalFilesystemToS3Operator(
            ...
            filename='{}/{}'.format(local_path, file_name),
            dest_key=s3_folder_name + '/' + file_name,
        ...
        try:
            ...
            os.remove('{}/{}'.format(local_path, file_name))
    ...
    try:
        ...
        for index, row in df.iterrows():
            ...
            for item in items:
                ...
                file_name = item['name']
                task_id = f"Downloading_file_{file_name}"
                ...
                transfer_file_from_gdrive_to_s3(task_id=task_id, folder_id=parent_folder_id, file_name=file_name, local_path='/tmp',
                                                bucket_name=S3_BUCKET_NAME, s3_prefix_start=S3_PREFIX_START, unique_load_name=row['unique_load_name'])
                current_date_time = datetime.now()
                ...
                load_files_from_S3_to_RedShift_one_file_at_a_time(dag_id=dag_id, run_id=run_id, unique_load_name=row['unique_load_name'], source_file_type=file_extension, target_table=row['target_table'],
                                                                  target_schema=row['target_schema'], file_name=file_name)
    except Exception as err:
        print(err)
...
def load_files_from_S3_to_RedShift_one_file_at_a_time(**kwargs):
    ...
    fil = S3_PREFIX_FOLDER + '/'+unique_load_name+'/'+file_name
    ...
    for col_name in required_column_list:
        if col_name not in file_columns:
            print('Column Name ' + col_name + ' Not present in file ' + fil)
    ...
    load_data_task = S3ToRedshiftOperator(
        ...
        copy_options=["CSV", "IGNOREHEADER 1", "IGNOREBLANKLINES"],
        aws_conn_id=S3_CONN_ID,
    ...
    p_load_process_reference_id = return_load_process_reference_id(schema_name='gdrive_ingestions', sp_name='gdrive_process_get_load_reference_id', params={
                                                                   'p_unique_load_name': unique_load_name, 'p_file_name': fil.split('/')[-1], 'rs_out': 'rs_out'})
