...
S3_PREFIX_START = 'gdrive_files/s3_files/kuda_statements'
GDRIVE_CONN = "gdrive_service_account"

account_numbers =[ '1100621302']

def get_last_max_date(account_number):
    redshift_hook = RedshiftSQLHook(redshift_conn_id=REDSHIFT_CONN_ID)
    sql = f"select id, file_name from s3_ingestions.load_process_reference where file_name like '%kuda_statements_{account_number}_%'  and processed = True"
...
def kuda_statements_api_download_as_csv(params,**kwargs):
    try:
        ...
        for account_number in account_numbers:         
            try:
                ...
                if pd.notna(prev_max_date) and override_dates.lower() != 'y' and (datetime.strptime(from_date,'%Y-%m-%d') - prev_max_date).days > 0:
                    from_date = datetime.strftime(prev_max_date ,'%Y-%m-%d')
                ...
                url = f"https://settlements-engine.private.paystack.co/providers/statement?bank_code=50211&from_date={from_date}&to_date={to_date}&page_number={page_number}&page_size={page_size}&account_number={account_number}"
                print(url)
                
                for attempt in range(1, max_retries + 1):
                    try:
                        data = pd.read_json(url)
                        total_records = data['data']['Data']['TotalRecordInStore']
                        records_data = data['data']['Data']['PostingsHistory']
                ...
                while page_number < total_calls_needed:
                    ...
                    for attempt in range(1, max_retries + 1):
                        try:
                            data = pd.read_json(url)
                            records_data.extend( data['data']['Data']['PostingsHistory'])
                            break  # If successful, exit the loop
                        except (requests.exceptions.HTTPError,KeyError) as e:
                            print(f"Attempt {attempt} failed with error: {e}")
                            if attempt < max_retries:
                ...
                filename =  'kuda_statements_'+account_number+'_'+from_date.replace('-','')+'_'+to_date.replace('-','')+'.csv'
                
                transactions.columns = [
                    (''.join([i for i in c if i not in punctuation.replace('_', '')])).lower().strip().replace(' ', '_')
                    for c in transactions.columns]
                transactions.loc[:, 'realdate'] = pd.to_datetime(transactions.loc[:, 'realdate'], errors='coerce').astype('datetime64[s]')
                transactions['referencenumber'] = transactions['referencenumber'].str.replace('R','').str.replace('A','')
                transactions[['referencenumber', 'accountnumber']] = transactions[
                    ['referencenumber', 'accountnumber']].apply(pd.to_numeric)
                transactions[['reversalreferencenumber', 'linkedaccountnumber']] = transactions[
                    ['reversalreferencenumber', 'linkedaccountnumber']].astype(str)
        ...
        print(err)
        message = f"Function kuda_statements_api_download_as_csv failed : \n \n" + str(err)
        send_notification(type="error", message=message)
        raise AirflowException(message)
...
with DAG(dag_id="kuda_statements_api_download_as_csv",
         description="kuda_statements_api_download_as_csv",
    ...
    kuda_statements_api_download_as_csv = PythonOperator(task_id='kuda_statements_api_download_as_csv',
                                  python_callable=kuda_statements_api_download_as_csv,
                                  op_kwargs={'local_path': '/tmp'},
                                  )
    dummy_start >> kuda_statements_api_download_as_csv 
