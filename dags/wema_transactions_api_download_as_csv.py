...
def date_range(START_DATE_TIME,END_DATE_TIME):
    required_date = START_DATE_TIME
    while required_date <= END_DATE_TIME:
        yield required_date
        if DEBUG:
            return
            ...
            for direction in directions:       
                try:
                    if override_dates.lower() == 'y':
                        ...
                        if pd.notna(prev_max_date_time) and override_dates.lower() != 'y' and diff_in_hours >= 2:
                            latest_start_date_time = prev_max_date_time + timedelta(hours=1) 
                        else:
                            latest_start_date_time = current_time_minus_required_hours 
                        latest_end_date_time_should_be = current_time_minus_required_hours
                        print(f'Start date used is : {latest_start_date_time}')
                        print(f'Final End date used is : {latest_end_date_time_should_be}')  
                    for single_date_time in date_range(latest_start_date_time,latest_end_date_time_should_be):
                        ...
                        asyncio.run(async_call_for_single_date(start_date=start_date,start_time=start_time,end_date=end_date,end_time=end_time,partner_id=partner_id,direction=direction))
                except Exception as err:
                    print(err)
...
with DAG(dag_id="wema_transactions_api_download_as_csv",
         description="wema_transactions_api_download_as_csv",
