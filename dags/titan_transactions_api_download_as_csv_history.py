        ...
        try:
            ...
            if r.ok is True:
                if int(json.loads(r.text)['responseCode']) == 2 and json.loads(r.text)['responseMessage'] in ['Transaction list is empty','Account number is invalid','Invalid date']:
                    ...
                    with open('/tmp/titan_transactions_unprocessed.txt','a') as unpr:
...
def call_for_single_date(account_number,start_date,end_date):
    ...
    try:
        ...
        #filename = f"titan_transactions_{account_number}_"+start_date.replace('-','')+'_'+end_date.replace('-','')+'.csv'
        ...
        upload_file_to_s3(task_id=task_id, file_name_with_path=filename_all, local_path='/tmp',s3_bucket_name=S3_BUCKET_NAME, s3_prefix_start=S3_PREFIX_START)                
...
def titan_transactions_api_download_as_csv_history(params,**kwargs):
    try:
        ...
        for index, acc in accounts_df.iterrows():
            ...
            inception_date = datetime.strftime(acc['inception_date'], '%Y-%m-%d')
        ...
        print(err)
        message = f"Function titan_transactions_api_download_as_csv_history failed : \n \n" + str(err)
        send_notification(type="error", message=message)
        raise AirflowException(message)
...
with DAG(dag_id="titan_transactions_api_download_as_csv_history",
         description="titan_transactions_api_download_as_csv_history",
    ...
    titan_transactions_api_download_as_csv_history = PythonOperator(task_id='titan_transactions_api_download_as_csv_history',
                                  python_callable=titan_transactions_api_download_as_csv_history,
                                  op_kwargs={'local_path': '/tmp'},
                                  )
    dummy_start >> titan_transactions_api_download_as_csv_history
